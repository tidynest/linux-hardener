...
if [[ "$INTERACTIVE" != "true" ]]; then
    ...
    echo ""
    echo -e "  IMPORTANT: the auth-success arm above just authenticated, and"
    echo -e "  auth_admin_keep caches that for about five minutes. A preview"
    echo -e "  inside that window raises NO dialog and goes straight to the"
    echo -e "  review step - the fix working, not a cancel test. Wait five"
    echo -e "  minutes before doing this part, so a dialog can appear."
    echo ""
