...
#
# Usage: sudo ./scripts/test/run-all-tests-parallel.sh [OPTIONS]
#
# Options:
...
Examples:
  ...
  ./scripts/test/run-all-tests-parallel.sh --no-cli --no-gui
...
if [[ $EUID -ne 0 ]] && [[ "$NEEDS_ROOT" == "true" ]]; then
    ...
    echo -e "  Or run ./scripts/test/run-desktop-tests.sh for desktop tests only"
    exit 1
fi
...
run_gui_tests() {
    ...
    "$SCRIPT_DIR/gui/run-gui-tests.sh" \
...
run_desktop_tests() {
    ...
    if [[ $EUID -eq 0 ]] && [[ -n "$SUDO_USER" ]]; then
        su - "$user" -c "cd '$PROJECT_DIR' && ./scripts/test/run-desktop-tests.sh" \
            > "$logfile" 2>&1
    else
...
if [[ "$USE_KITTY" == "true" ]]; then
    ...
    if [[ "$DO_GUI" == "true" ]]; then
        run_in_kitty "GUI Tests" \
            "$SCRIPT_DIR/gui/run-gui-tests.sh --parallel --jobs $MAX_JOBS" \
