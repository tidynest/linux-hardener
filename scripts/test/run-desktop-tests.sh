...
run_ux_tests() {
    ...
    "$PROJECT_DIR/scripts/test/gui/tauri-ux-test.sh"
    return $?
}

run_fn_tests() {
    echo -e "${CYAN}[FN] Running functional tests (scans, reports)...${NC}"
    "$PROJECT_DIR/scripts/test/gui/tauri-functional-test.sh"
    return $?
}
...
if [[ "$USE_KITTY" == "true" ]]; then
    exec kitty --title "Desktop Tests" bash -c "
        cd '$PROJECT_DIR'
        ./scripts/test/run-desktop-tests.sh ${UX_ONLY:+--ux-only} ${FN_ONLY:+--fn-only} ${NO_CLEANUP:+--no-cleanup}
        echo ''
