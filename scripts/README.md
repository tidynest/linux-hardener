...
| `test/` | Host-side test suites and orchestrators: cross-distro, package-install, root/full suites, desktop tests, rollback verification, parallel runner, plus `release-readiness-root.sh` which batches every root-only suite into one invocation, and `test-readiness-summary.sh`, the one unprivileged unit test of that batcher's summary carry-forward |
...
| **Readiness summary unit test** | `./scripts/test/test-readiness-summary.sh` (unprivileged, ~1s) |
...
| Directory Modes | `validate_directory_modes.py` | The mode of `/etc/linux-hardener`, `/var/lib/linux-hardener` and `/var/log/linux-hardener` against `packaging/PKGBUILD`, across every site that states one: the rpm spec twice, `debian/rules`, `debian/postinst`, the package suite's install and its assertions, the commands `installation.md` tells a reader to type, the architecture table and a Rust doc comment. Ten sites, twenty-six statements. `94a1a98e` moved `/var/lib` to 0700 in three of them and missed the rest; `test-package-install.sh` both installs and asserts the mode from one file, so it agreed with itself while disagreeing with everything that ships and kept passing. Two documents were still wrong four commits later, one of them a command a reader would run by hand. Each site declares which directories it must cover, so a site that stops mentioning one fails rather than quietly shrinking, and a tracked file that starts stating a mode without being registered fails too. Modes are compared as octal, so `0755` in prose and `755` in a shell command are the same value. The Rust that enforces these at runtime is not read: both call sites take the mode as an argument, so a static parse would pin the argument rather than the behaviour |
