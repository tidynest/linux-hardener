...
| Evidence Ledger | `validate_evidence_ledger.py` | Every path `docs/reference/evidence-ledger.md` cites in its Evidence column still exists, because a row whose file was renamed or deleted goes on asserting coverage that is gone, and nothing else in the tree reads those paths as anything but prose. A ledger yielding no citations at all is failed rather than passed, since a rewritten table would otherwise slip through as a vacuous success. Whether the named test exercises the claim beside it is a review-time judgement and is not covered |
