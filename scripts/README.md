...
**Purpose**: Host orchestrator that runs the Playwright Web UI suite across every distro in `DISTRO_ORDER`. The suite **declares 156 tests in 11 files** as of 2026-08-18 and its last result is **154 of 154** on all six distributions on 2026-08-16 at `5b715039`, none failed, skipped or flaky, with every container destroyed and recreated first, Ubuntu included. The two figures differ on purpose: `T-FLEET-10` and `T-SCHED-07` were added after that run and have never been executed. Recorded in [distribution-validation.md](../docs/reference/distribution-validation.md). The count is still mostly generated rather than literal, which is why `npx playwright test --list` is the way to read it: `themes.spec.js` produces 35 from 7 themes x 5 states, and `hardening.spec.js`'s T-DIVG-03 produces 2 from two viewport widths. For each distro, copies the WASM build and test files into the container, then delegates to `gui-test-inner.sh` via `systemd-nspawn --pipe`.
