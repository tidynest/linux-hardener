...
**Purpose**: Host orchestrator that runs the Playwright Web UI suite across every distro in `DISTRO_ORDER`. The suite is **171 tests in 11 files**, and that is the tree rather than a result: the last sweep ran **168 of 168** on all six distributions on 2026-08-27, none failed, skipped or flaky, one worker and no name filter, Ubuntu included, 44 screenshots each. `T-CONF-11`, `T-CONF-12` and `T-CONF-13` were written after it and have never executed. That sweep was the first to execute `T-FLEET-11`, which had been authored unexecuted the day before with the fleet profile badge. Recorded in [distribution-validation.md](../docs/reference/distribution-validation.md). The count is still mostly generated rather than literal, which is why `npx playwright test --list` is the way to read it: `themes.spec.js` produces 42 from 7 themes x 6 states, and `hardening.spec.js`'s T-DIVG-03 produces 2 from two viewport widths. For each distro, copies the WASM build and test files into the container, then delegates to `gui-test-inner.sh` via `systemd-nspawn --pipe`.
