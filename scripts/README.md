...
**Purpose**: Host orchestrator that runs the Playwright Web UI suite across every distro in `DISTRO_ORDER`. The suite is **156 tests in 11 files**, and that is a result rather than a static count: it ran 156 of 156 on all six distributions on 2026-08-18 at `653b4ff1`, none failed, skipped or flaky, with every container destroyed and recreated first, Ubuntu included. Recorded in [distribution-validation.md](../docs/reference/distribution-validation.md). The count is still mostly generated rather than literal, which is why `npx playwright test --list` is the way to read it: `themes.spec.js` produces 35 from 7 themes x 5 states, and `hardening.spec.js`'s T-DIVG-03 produces 2 from two viewport widths. For each distro, copies the WASM build and test files into the container, then delegates to `gui-test-inner.sh` via `systemd-nspawn --pipe`.
