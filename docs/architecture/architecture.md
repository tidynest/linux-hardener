...
|-------|---------|-------------|
| `hardener-types` | WASM-compatible shared types (5 source files: `lib.rs`, `config_picker.rs`, `remote.rs`, `scheduler.rs`, `checkpoint.rs`; plus three `tests.rs` split out under issue #49) | `PluginId`, `Severity`, `Finding`, `ScanResult`, `UncheckedCheck`, `ApplyResult`, `ComplianceReport`, `RollbackResult`, `ValidationReport`, `ConfigSummary`, `RemoteHostProfile`, `RemoteConnectionStatus`, `RemoteConnectionInfo`, `HostsConfig`, `SchedulerUiConfig`, `NotificationUiConfig`, `TestNotificationResult`, `FleetHostScan`, `FleetHostStatus`, `SeverityTallies` |
...
(`src-tauri/capabilities/default.json`) grants all 32 permissions explicitly,
ordered by risk tier (read-only, mutating-local, remote, config), so revoking a
single command means deleting one line. **The number here is a fourth copy of
the command count** and no validator reads it: `validate_tauri_docs.py` parses
signatures out of `file-map.md` alone. Re-read it with
`grep -cE '"allow-' src-tauri/capabilities/default.json`, which must equal the
`COMMANDS` list in `build.rs` and the `generate_handler!` block in `main.rs`;
all three read 32 on 2026-08-18, and this sentence said 30 from 2026-07-19. The command list in `build.rs` must stay
