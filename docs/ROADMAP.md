...
| nftables ruleset persistence | The nftables backend renders its ruleset into `/etc/nftables.conf` and enables the unit, so the rules survive a reboot wherever `nftables.service` reads that path. Fedora and RHEL read `/etc/sysconfig/nftables.conf` instead, so persistence there is still missing | High | ⬜ Open, issue #52 |
