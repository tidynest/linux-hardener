...
| Tests the default suite runs | `cargo nextest run --workspace` | 1899 passed, 40 skipped |
| Tests `cargo test` runs, doctests included | `cargo test --workspace`, summing every `test result:` line | 1899 passed, 0 failed, 47 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1939 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1939 across 295 files |

The gap between 1939 annotations and 1899 executions is exactly 40, and all 40
...
are the same number, 1939, and they are meant to be: the check globs every `.rs`
...
covers. 1899 + 6 = 1905 and 40 + 7 = 47.
