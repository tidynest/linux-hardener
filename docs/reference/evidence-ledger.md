...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1941 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1941 across 295 files |

The gap between 1941 annotations and 1901 executions is exactly 40, and all 40
...
are the same number, 1941, and they are meant to be: the check globs every `.rs`
...
covers. 1901 + 6 = 1907 and 40 + 7 = 47.
