...
| Tests the default suite runs | `cargo nextest run --workspace` | 2204 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2210 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2246 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2246 across 309 files |
...
The gap between 2246 annotations and 2204 executions is exactly 42, and all 42
...
are the same number, 2246, and they are meant to be: the check globs every `.rs`
...
covers. 2204 + 6 = 2210 and 42 + 7 = 49.
