...
| Tests the default suite runs | `cargo nextest run --workspace` | 2158 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2164 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2200 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2200 across 305 files |
...
The gap between 2200 annotations and 2158 executions is exactly 42, and all 42
...
are the same number, 2200, and they are meant to be: the check globs every `.rs`
...
covers. 2158 + 6 = 2164 and 42 + 7 = 49.
