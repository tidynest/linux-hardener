...
| Tests the default suite runs | `cargo nextest run --workspace` | 1997 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2003 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2039 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2039 across 299 files |
...
The gap between 2039 annotations and 1997 executions is exactly 42, and all 42
...
are the same number, 2039, and they are meant to be: the check globs every `.rs`
...
covers. 1997 + 6 = 2003 and 42 + 7 = 49.
