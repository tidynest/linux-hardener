...
- **`scripts/test/differential-suite.sh` applies six plugins at most.** Its
  ...
  `apply --all`, whose sole check greps the aggregate output for a summary line,
  so nothing reads mac's apply back at all. What a container skips is that
  plugin's dedicated section 14 apply, not its apply as such: `apply --all`
  selects every registered plugin, so mac's apply does run there, checkpoints
  its config paths, and on a container that carries SELinux or AppArmor tries
  to raise the mode. Its scan gets an unconditional exit-status check in section
