...
| Tests the default suite runs | `cargo nextest run --workspace` | 2319 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2325 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2361 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2361 across 338 files |
...
The gap between 2361 annotations and 2319 executions is exactly 42, and all 42
...
covers. 2319 + 6 = 2325 and 42 + 7 = 49.
