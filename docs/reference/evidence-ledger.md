...
| Tests the default suite runs | `cargo nextest run --workspace` | 1810 passed, 40 skipped |
| Tests `cargo test` runs, doctests included | `cargo test --workspace`, summing every `test result:` line | 1816 passed, 0 failed, 47 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1850 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1850 across 281 files |

The gap between 1850 annotations and 1810 executions is exactly 40, and all 40
...
are the same number, 1850, and they are meant to be: the check globs every `.rs`
...
covers. 1810 + 6 = 1816 and 40 + 7 = 47.
...
- **Nothing automated runs above grade 2.** `.github/workflows/ci.yml` runs
  ...
  strictly smaller than the 1810 recorded above, so a green CI run is a weaker
