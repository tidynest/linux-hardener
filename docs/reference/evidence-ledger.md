...
| Tests the default suite runs | `cargo nextest run --workspace` | 2032 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2038 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2074 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2074 across 299 files |
...
The gap between 2074 annotations and 2032 executions is exactly 42, and all 42
...
are the same number, 2074, and they are meant to be: the check globs every `.rs`
...
covers. 2032 + 6 = 2038 and 42 + 7 = 49.
