...
| Tests the default suite runs | `cargo nextest run --workspace` | 1784 passed, 40 skipped |
| Tests `cargo test` runs, doctests included | `cargo test --workspace`, summing every `test result:` line | 1790 passed, 0 failed, 47 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1824 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1824 across 281 files |

The gap between 1824 annotations and 1784 executions is exactly 40, and all 40
...
are the same number, 1824, and they are meant to be: the check globs every `.rs`
...
covers. 1784 + 6 = 1790 and 40 + 7 = 47.
...
- **Nothing automated runs above grade 2.** `.github/workflows/ci.yml` runs
  ...
  strictly smaller than the 1784 recorded above, so a green CI run is a weaker
