...
| Tests the default suite runs | `cargo nextest run --workspace` | 1893 passed, 40 skipped |
| Tests `cargo test` runs, doctests included | `cargo test --workspace`, summing every `test result:` line | 1897 passed, 0 failed, 47 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1933 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1933 across 295 files |

The gap between 1933 annotations and 1893 executions is exactly 40, and all 40
...
are the same number, 1933, and they are meant to be: the check globs every `.rs`
...
covers. 1893 + 6 = 1899 and 40 + 7 = 47.
