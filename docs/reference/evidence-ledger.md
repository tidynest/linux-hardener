...
| Tests the default suite runs | `cargo nextest run --workspace` | 1895 passed, 40 skipped |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1935 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1935 across 295 files |

The gap between 1935 annotations and 1895 executions is exactly 40, and all 40
...
are the same number, 1935, and they are meant to be: the check globs every `.rs`
...
covers. 1895 + 6 = 1901 and 40 + 7 = 47.
