...
| Tests the default suite runs | `cargo nextest run --workspace` | 1897 passed, 40 skipped |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1937 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1937 across 295 files |

The gap between 1937 annotations and 1897 executions is exactly 40, and all 40
...
are the same number, 1937, and they are meant to be: the check globs every `.rs`
...
covers. 1897 + 6 = 1903 and 40 + 7 = 47.
