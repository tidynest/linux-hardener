...
| Tests the default suite runs | `cargo nextest run --workspace` | 2035 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2041 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2077 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2077 across 299 files |
...
The gap between 2077 annotations and 2035 executions is exactly 42, and all 42
...
are the same number, 2077, and they are meant to be: the check globs every `.rs`
...
covers. 2035 + 6 = 2041 and 42 + 7 = 49.
