...
| Tests the default suite runs | `cargo nextest run --workspace` | 2040 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2046 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2082 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2082 across 299 files |
...
The gap between 2082 annotations and 2040 executions is exactly 42, and all 42
...
are the same number, 2082, and they are meant to be: the check globs every `.rs`
...
covers. 2040 + 6 = 2046 and 42 + 7 = 49.
