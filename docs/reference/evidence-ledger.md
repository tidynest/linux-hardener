...
| Tests the default suite runs | `cargo nextest run --workspace` | 2263 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2269 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2305 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2305 across 316 files |
...
The gap between 2305 annotations and 2263 executions is exactly 42, and all 42
...
are the same number, 2305, and they are meant to be: the check globs every `.rs`
...
covers. 2263 + 6 = 2269 and 42 + 7 = 49.
