...
| Tests the default suite runs | `cargo nextest run --workspace` | 2003 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2009 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2045 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2045 across 299 files |
...
The gap between 2045 annotations and 2003 executions is exactly 42, and all 42
...
are the same number, 2045, and they are meant to be: the check globs every `.rs`
...
covers. 2003 + 6 = 2009 and 42 + 7 = 49.
