...
| Tests the default suite runs | `cargo nextest run --workspace` | 1924 passed, 40 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 1930 passed, 0 failed, 47 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1964 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1964 across 295 files |

The gap between 1964 annotations and 1924 executions is exactly 40, and all 40
...
are the same number, 1964, and they are meant to be: the check globs every `.rs`
