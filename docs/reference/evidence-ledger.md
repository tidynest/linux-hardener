...
## Baseline, as measured on 2026-09-02
...
|---|---|---|
| Workspace version measured | `grep -m1 '^version' Cargo.toml` | 1.8.2 |
| Tests the default suite runs | `cargo nextest run --workspace` | 2343 passed, 42 skipped |
| Tests `cargo test` runs, doctests included (nextest total plus 6 doctests) | `cargo test --workspace`, summing every `test result:` line | 2349 passed, 0 failed, 49 ignored |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 2385 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 2385 across 340 files |
...
The gap between 2385 annotations and 2343 executions is exactly 42, and all 42
...
are the same number, 2385, and they are meant to be: the check globs every `.rs`
...
covers. 2343 + 6 = 2349 and 42 + 7 = 49.
...
- **Nothing automated runs above grade 2.** `.github/workflows/ci.yml` runs
  ...
  strictly smaller than the 2142 recorded above, so a green CI run is a weaker
