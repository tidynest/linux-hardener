...
| Tests the default suite runs | `cargo nextest run --workspace` | 1891 passed, 40 skipped |
...
| Test annotations in the tree | `grep -rEc '^\s*#\[(tokio::)?test\]' crates src-tauri` summed | 1931 |
| Tests the assertion check reads | `python3 scripts/validate/validate_test_assertions.py --all` | 1931 across 295 files |

The gap between 1931 annotations and 1891 executions is exactly 40, and all 40
...
are the same number, 1931, and they are meant to be: the check globs every `.rs`
...
covers. 1891 + 6 = 1897 and 40 + 7 = 47.
