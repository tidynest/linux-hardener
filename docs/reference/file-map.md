...
The table covers the ten crates under `crates/` and sums to 2173. The eleventh
workspace member, `src-tauri`, carries 187 more, which is why the tree total the
evidence ledger records is 2360 and not this table's sum.

| Crate | Unit Tests | Integration Tests | Annotations |
|-------|------------|-------------------|-------------|
| hardener-common | `error.rs`, `file_utils.rs`, `binary_utils.rs`, `text.rs`, `vendor_config.rs`, `executor/mod.rs`, `executor/mock.rs` | `common_types.rs`, `error_tests.rs`, `file_utils_tests.rs`, `common/mod.rs` | 138 |
...
| hardener-ui | `utils/mod.rs`, `utils/theme.rs`, `pages/fleet_apply_page.rs`, `components/configure_section.rs`, `components/adhoc_host_input.rs` | - | 147 |
