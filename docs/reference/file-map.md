...
| `src/cli/tests.rs` | Unit tests for `src/cli.rs`, 36 tests of argument parsing | Test-only; `super` resolves to `crate::cli`, so its imports carried across unchanged |
...
| hardener-cli | `cli.rs`, `output.rs`, `ssh_config.rs`, and ten of `commands/`: `apply.rs`, `batch.rs`, `checkpoint.rs`, `history.rs`, `plugin_filter.rs`, `privilege.rs`, `report.rs`, `report_wizard.rs`, `scan.rs`, `state.rs` | `batch_ssh_integration.rs` (live-sshd, `#[ignore]`), `ssh_refusal.rs` (drives the built binary) | 185 |
