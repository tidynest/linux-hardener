...
| `src/commands/exception/document/tests.rs` | Unit tests for `src/commands/exception/document.rs`, 8 tests | Test-only; `super` resolves to `crate::commands::exception::document`, so its imports carried across unchanged |
