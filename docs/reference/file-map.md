...
| File | Purpose | Key Exports |
|------|---------|-------------|
| `index.html` | Entry HTML; no web fonts (the Tauri CSP blocks them, the stacks in `styles.css` name local faces) | `#app` mount point |
| `styles.css` | Base styles plus the 7-theme system (`[data-theme="..."]` overrides, incl. light Daywatch and WCAG AAA High Contrast) | CSS Variables, utility classes (.truncate, .sr-only, .skip-link), tabs, sidebar, score hero, buttons, tables, forms |
