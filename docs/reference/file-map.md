...
The table covers the ten crates under `crates/` and sums to 2068. The eleventh
workspace member, `src-tauri`, carries 109 more, which is why the tree total the
evidence ledger records is 2177 and not this table's sum.
...
| hardener-core | `config.rs`, `config/scope.rs`, `config_loader.rs`, `config_validation.rs`, `plugin.rs`, `inventory.rs`, `executor/local.rs`, `executor/ssh.rs` | `config_tests.rs`, `context_tests.rs`, `inventory_shared_path.rs`, `mock_executor_tests.rs`, `plugin_manager_tests.rs`, `registry_tests.rs`, `ssh_executor_tests.rs` | 215 |
