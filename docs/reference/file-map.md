...
The table covers the ten crates under `crates/` and sums to 1746. The eleventh
workspace member, `src-tauri`, carries 104 more, which is why the tree total the
evidence ledger records is 1855 and not this table's sum.
...
| hardener-plugins | `lib.rs`, `strictness.rs`, `scan_outcome.rs`, `shell_config.rs`, and all eight plugin modules (`ssh/dropin.rs`, `ssh/include.rs`, `kernel/divergence.rs` and `firewall/divergence.rs` also carry their own) | `*_tests.rs` (8 files), `*_mock_tests.rs` (8 files), `ssh_integration_tests.rs`, `common/mod.rs` | 759 |
