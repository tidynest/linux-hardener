...
| `src/commands/compliance.rs` | Compliance report generation and export, and the source selection deciding between a persisted scan and a fresh one | `generate_compliance_report`, `export_compliance_report`, `collect_findings`, `latest_or_fresh_findings`, `export_destination` |
