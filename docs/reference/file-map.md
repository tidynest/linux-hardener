...
The table covers the ten crates under `crates/` and sums to 2162. The eleventh
workspace member, `src-tauri`, carries 187 more, which is why the tree total the
evidence ledger records is 2349 and not this table's sum.
...
| hardener-ui | `utils/mod.rs`, `utils/theme.rs`, `pages/fleet_apply_page.rs`, `components/configure_section.rs`, `components/adhoc_host_input.rs` | - | 140 |
