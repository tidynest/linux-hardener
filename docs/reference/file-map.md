...
| `src/commands/exception/document.rs` | Pure TOML-text edits of one `[<section>.exceptions."<key>"]` table, `toml_edit`-based so an operator's comments, formatting and unrelated sections survive the write untouched (the reason `save_scheduler_config` at `src-tauri/src/commands/scheduler.rs` already gives). No file IO, no scanning, no clap: those live in `exception.rs`. A freshly created `exceptions` table is marked implicit, so it contributes no empty `[<section>.exceptions]` header of its own | `upsert_exception()`, `remove_exception()` |
...
| `src/output/tests.rs` | Unit tests for `src/output.rs`, 43 tests of the renderers | Test-only; `super` resolves to `crate::output`, so its imports carried across unchanged |
...
| `src/fuzz_seams.rs` | Re-exports of the configuration parsers for the fuzz targets under `fuzz/`, compiled only under cargo-fuzz's `--cfg fuzzing`: the parsers stay unreachable from every other consumer, so the crate's public surface is unchanged where it ships | `ssh_include`, `pam_parsing`, `nftables_include` |
...
| `src/ssh/assess.rs` | Network | The body of `SshHardeningPlugin::scan`, extracted so the trait impl in `mod.rs` stays whole (a trait impl cannot span files) and the plugin's file count matches its seams. Reads whichever sshd_config layer is in force, resolves includes in sshd's own order, and reports the directives whose effective values violate the baseline | `scan()` |
| `src/ssh/apply.rs` | Network | The body of `SshHardeningPlugin::apply`. Locks and checkpoints the config path the layered reader resolved, backs it up, writes the managed directives (the drop-in where the main file cannot win), gates every commit on `sshd -t`, and restarts sshd only after the candidate passed | `apply()` |
| `src/ssh/validate.rs` | Network | The body of `SshHardeningPlugin::validate`, the dry-run preview. Reads through the same layered reader scan and apply use, so a preview cannot name a different file than the run it previews | `validate()` |
...
| `src/pam/parsing.rs` | Auth | The pure parsing halves of the PAM readers (`stack_loads_module`, `inline_arg_in_content`, `apply_exact_directive`), extracted so the fuzz targets can hold them to their invariants without a live host. The async readers in `mod.rs` call in; `pub(crate)` so the crate's fuzz-seam can re-export them | `stack_loads_module()`, `inline_arg_in_content()`, `apply_exact_directive()` |
| `src/pam/assess.rs` | Auth | The body of `PamHardeningPlugin::scan`, extracted beside `mod.rs`'s whole trait impl. Classified reads of every managed file, directive findings, layer drift and module presence | `scan()` |
| `src/pam/apply.rs` | Auth | The body of `PamHardeningPlugin::apply`. Checkpoints the five managed paths, rewrites pwquality.conf and login.defs through the backup-and-write path, and refuses to edit the PAM stack itself | `apply()` |
| `src/pam/validate.rs` | Auth | The body of `PamHardeningPlugin::validate`, the dry-run preview through the same module-presence, drift and classified readers scan and apply use | `validate()` |
...
| `src/scan_evidence/tests.rs` | Unit tests for `src/scan_evidence.rs`, 14 tests: the coverage an incomplete scan carries, a disabled plugin not reading as a failed one, a full scan contributing nothing, an absent plugin being reported unassessed, disabled against uncovered, the three blockers, a result from an unknown plugin, both directions of an unenumerable registry, the assessed set being a deduplicated union, a skip marker routing by its reason rather than through the failure arm, a marker for an unregistered plugin standing in for nothing, and a plugin both marked and passed as skipped yielding one stand-in | Test-only; ported from `hardener-plugins/src/scan_outcome/tests.rs` with the flatten. They build their inventory by hand, so the assertions do not move when the real eight plugins change what they declare |
...
| File | Purpose | Key Exports |
|------|---------|-------------|
| `index.html` | Entry HTML; a `copy-dir` link bundles `fonts/` into `dist/` (a remote font stylesheet would be blocked by the Tauri CSP) | `#app` mount point |
| `fonts/` | Martian Mono and IBM Plex Sans, variable woff2, latin subset, OFL licences beside them; declared by the two `@font-face` rules at the top of `styles.css` | two `.woff2`, two licence texts |
| `styles.css` | Base styles plus the 7-theme system (`[data-theme="..."]` overrides, incl. light Daywatch and WCAG AAA High Contrast); the ink frame (posture strip plus sidebar on `--bg-ink`), the score hero and the hardening area map | CSS Variables, utility classes (.truncate, .sr-only, .skip-link), tabs, sidebar, posture strip, score hero, area map, buttons, tables, forms |
...
| `src/state/mod.rs` | Reactive state | `AppState` (24 signals; `last_scan_completed_at` is the history-backed last-scan stamp one `Effect` in `App` owns), `unchecked_tally()`, `SchedulerForm` (the lifted Scheduler-page form bundle, imported by `pages/scheduler_page.rs` and `components/notification_section.rs`) |
...
| File | Purpose | Key Exports |
|------|---------|-------------|
| `src/pages/dashboard_page.rs` | Dashboard: header, security score hero, hardening area map, recent activity | `DashboardPage` |
...
| `src/components/posture_strip.rs` | Read-only statusline above `<main>` on every route: score and band, live critical and high counts, the last-scan stamp, a scanning segment. No button, heading or live region, for the test reasons in its module doc | `PostureStrip` |
| `src/components/area_map.rs` | Dashboard area map: one tile per hardening area in Hardening page order, banded by the worst live severity, with not-scanned, disabled-by-config and scan-failed states that cannot pass for clean; pure half unit-tested on the host | `AreaMap`, `area_tiles()`, `live_counts()`, `AreaTile`, `AreaStatus`, `LiveCounts` |
...
| `src/commands/mod.rs` | Shared plumbing for every command, split from the flat `commands.rs` this table used to carry as one row: error sanitisation for the GUI (CWE-209), the single-flight guard around privileged operations, `RemoteState`/`ActiveConnection`, config and inventory paths, the validated hardener-binary lookup, `run_privileged` and its JSON acceptance, database and manager factories, `persist_scan_results`, `fail_session_on_err`, `scan_selection_refusal`. Declares the nine domain modules below and re-exports their commands | `sanitise_error`, `PrivilegedOpGuard`, `run_privileged`, `create_checkpoint_manager`, `fail_session_on_err`, `scan_selection_refusal` |
| `src/commands/scan.rs` | Local scanning, apply, dry-run and rollback commands, plus the CLI-shaped entry type they parse | `run_scan`, `run_deep_scan`, `run_apply`, `run_apply_dry_run`, `run_rollback`, `list_plugins`, `get_latest_scan`, `CliScanEntry::into_scan_result` |
| `src/commands/checkpoint.rs` | Checkpoint listing, creation, deletion and detail, the reachability rule behind the list, and the guard deciding whether a deletion is worth an authentication prompt | `get_checkpoints`, `create_checkpoint`, `delete_checkpoint`, `get_checkpoint_detail`, `collect_checkpoints`, `restorable_here`, `resolve_delete` |
| `src/commands/exception.rs` | Policy-exception commands and the flag construction behind them | `add_policy_exception`, `remove_policy_exception`, `exception_add_args` |
| `src/commands/compliance.rs` | Compliance report generation and export, and the source selection deciding between a persisted scan and a fresh one | `generate_compliance_report`, `export_compliance_report` (both refuse through `selected_frameworks` when the selection names no framework the tool knows, before any scan is sourced), `collect_findings`, `latest_or_fresh_findings`, `export_destination` |
| `src/commands/history.rs` | Scan-session history and per-host history, with the row ceiling both obey | `get_scan_history`, `get_scan_session`, `get_host_history`, `session_to_info`, `scan_history_rows` |
| `src/commands/remote.rs` | Host inventory commands and the one live connection, plus the executor-shared scan helper | `list_remote_hosts`, `save_remote_host`, `delete_remote_host`, `connect_remote`, `disconnect_remote`, `upsert_host`, `remove_host`, `scan_with_executor` |
| `src/commands/fleet.rs` | Fleet scan, apply and rollback over the host inventory or ad-hoc targets: the bounded-concurrency orchestrator, posture folding, and the batch-argv builders | `run_remote_scan`, `run_fleet_scan`, `run_fleet_apply`, `run_fleet_rollback`, `scan_fleet`, `ssh_config_for`, `fleet_targets`, `run_fleet_mutation`, `build_batch_args` |
| `src/commands/scheduler.rs` | Scheduler settings read and write, and the test-notification verdict; the write overlays a TOML table through `toml_edit` so an operator's comments survive | `get_scheduler_config`, `save_scheduler_config`, `test_notification`, `write_scheduler_config`, `overlay`, `test_notification_verdict` |
| `src/commands/config.rs` | Config validation and the file picker, with the section summary the picker card reports | `validate_config`, `pick_config_file`, `plugin_sections`, `summarise_config` |
...
| `scripts/validate/validate_readme_claims.py` | README "Known limitations" validator: holds each bullet to the code that would make it false, three of five machine-checked and two registered as judgements |
...
| `scripts/test/ssh-ignored-suite.sh` | Boots the SSH fixture, reads the `export` line it prints, runs the three `#[ignore]` binaries that gate on `SSH_TEST_HOST` (`ssh_executor_tests`, `ssh_integration_tests`, `batch_ssh_integration`) and stops the machine; the tests gated on `NFTABLES_LIVE_APPLY_HOST` are skipped unless that variable is set, their names read from the test source at run time because they panic without it by design; one test thread per binary, since the tests share one sshd; wants a freshly created container |
...
The table covers the ten crates under `crates/` and sums to 2193. The eleventh
workspace member, `src-tauri`, carries 192 more, which is why the tree total the
evidence ledger records is 2385 and not this table's sum.

| Crate | Unit Tests | Integration Tests | Annotations |
|-------|------------|-------------------|-------------|
| hardener-common | `error.rs`, `file_utils.rs`, `binary_utils.rs`, `text.rs`, `vendor_config.rs`, `executor/mod.rs`, `executor/mock.rs` | `common_types.rs`, `error_tests.rs`, `file_utils_tests.rs`, `common/mod.rs` | 139 |
| hardener-compliance | `generator.rs`, `profiles.rs`, `frameworks/iso27001.rs`, `output/mod.rs`, and five of `output/`: `text.rs`, `json.rs`, `csv.rs`, `html.rs`, `pdf.rs` | `assessment_honesty.rs`, `config_tests.rs`, `framework_tests.rs`, `report_tests.rs` | 139 |
| hardener-state | `db.rs`, `hash_chain.rs`, `signing.rs`, `manager.rs`, `checkpoint.rs` | `audit_tests.rs`, `checkpoint_system.rs`, `db_tests.rs`, `scan_manager_tests.rs`, `signing_tests.rs`, `common/mod.rs` || 151 |
...
| hardener-ui | `utils/mod.rs`, `utils/theme.rs`, `pages/fleet_apply_page.rs`, `components/configure_section.rs`, `components/adhoc_host_input.rs`, `components/area_map.rs`, `components/security_score.rs` | - | 154 |
...
| `gui-tests/tests/rollback-modal.spec.js` | T-RBM-01..09 (9 tests): the rollback modal's lifecycle around that divergence section. The Confirm preview and its file kinds, Cancel proven not to have invoked anything (the mock's `__mockCalls` log), the unreadable-preview arm under `checkpoint_source=detail_denied`, the Restoring stage with Escape held inert by `rollback_mode=hold`, the cancelled-auth return, the hard-error banner under `rollback_mode=error`, the reload section (`rollback_reloads` was absent from the mock until 2026-09-01 and the section had never rendered), the `reloads_ok()` half of the failure header under `rollback_mode=reload_failed`, and Done. Authored 2026-09-01, first execution owed to the next sweep |
