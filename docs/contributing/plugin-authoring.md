...
- **`apply()`**: creates a checkpoint first (see below), then makes changes.
  ...
  `apply_success` should reflect whether every change succeeded.
...
contexts); record the returned ID in `apply_checkpoint_id`. For plugins that
only change modes or ownership,
...
SELinux nor AppArmor is detected it records a "nothing to configure
(skipped)" change with `change_success: true` and returns
`apply_success: true`, because many distributions legitimately ship without a
MAC system. Reserve `apply_success: false` for a subsystem that is present
but genuinely broken.
