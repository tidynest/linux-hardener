...
impl MockExecutor {
    ...
    pub fn with_file(self, path: &str, content: &str) -> Self {
        ...
        self.files.lock().expect("files mutex poisoned").insert(path_buf.clone(), content.to_string());
        // Auto-generate metadata for the file
        self.file_metadata.lock().expect("metadata mutex poisoned").insert(path_buf, FileMetadata {
            exists: true,
            is_file: true,
            is_dir: false,
            mode: 0o644,
    ...
    pub fn with_file_metadata(self, path: &str, content: &str, metadata: FileMetadata) -> Self {
        let path_buf = PathBuf::from(path);
        self.files.lock().expect("files mutex poisoned").insert(path_buf.clone(), content.to_string());
        self.file_metadata.lock().expect("metadata mutex poisoned").insert(path_buf, metadata);
        self
    }
    ...
    pub fn with_command(self, program: &str, args: &[&str], output: CommandOutput) -> Self {
        ...
        self.commands.lock().expect("commands mutex poisoned").insert(key, output);
        self
    }

    /// Registers whether a command exists.
    pub fn with_command_exists(self, program: &str, exists: bool) -> Self {
        self.command_exists.lock().expect("command_exists mutex poisoned").insert(program.to_string(), exists);
        self
    }
...
impl SystemExecutor for MockExecutor {
    ...
    async fn read_file(&self, path: &Path) -> Result<String> {
        self.log.lock().expect("log mutex poisoned").files_read.push(path.to_path_buf());
    ...
    async fn read_file_optional(&self, path: &Path) -> Result<Option<String>> {
        ...
        Ok(self.files.lock().expect("files mutex poisoned").get(path).cloned())
    }

    async fn write_file(&self, path: &Path, content: &str) -> Result<()> {
        self.log.lock().expect("log mutex poisoned").files_written.push((path.to_path_buf(), content.to_string()));
    ...
    async fn path_exists(&self, path: &Path) -> Result<bool> {
        Ok(self.file_metadata.lock().expect("metadata mutex poisoned").get(path).map(|m|
            m.exists).unwrap_or(false))
    }
    ...
    async fn execute_command(&self, program: &str, args: &[&str]) ->
    Result<CommandOutput> {
        let args_vec: Vec<String> = args.iter().map(|s| s.to_string()).collect();
        self.log.lock().expect("log mutex poisoned").commands_executed.push((program.to_string(), args_vec.clone()));
    ...
    async fn command_exists(&self, program: &str) -> Result<bool> {
        ...
        if let Some(&exists) = self.command_exists.lock().expect("command_exists mutex poisoned").get(program) {
            return Ok(exists);
        }
