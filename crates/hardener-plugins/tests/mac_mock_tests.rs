...
/// a MAC finding names a state (`selinux-not-enforcing`) while its exception
/// is keyed on the goal (`selinux-enforcing`), so the two are unrelated strings.
///
...
#[tokio::test]
async fn every_mac_finding_names_the_exception_key_that_silences_it() {
    let scan = async |config: &PluginConfig| {
        MacHardeningPlugin::new()
            .scan(
                &Context::with_executor(Arc::new(selinux_permissive_executor())),
                config,
            )
            .await
            .expect("mac scan should not error")
    };
    ...
    assert!(
        sampled.iter().any(|key| key == "selinux-enforcing"),
        "the SELinux mode finding is keyed on selinux-enforcing",
    );
