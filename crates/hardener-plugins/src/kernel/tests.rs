...
/// No file or command is registered on the mock, so `read_sysctl` fails for
/// every managed parameter, which is the cheapest reliable way to make the
/// owned-path half produce a row.
#[tokio::test]
async fn kernel_divergences_after_rollback_is_scoped_to_restored_sysctl_paths() {
    ...
    assert!(
        unrelated.is_empty(),
        "no restored path was under /etc/sysctl.conf or /etc/sysctl.d, so the probe must not \
         have run"
    );

    let owned = plugin
        .divergences_after_rollback(&ctx, &[std::path::PathBuf::from(SYSCTL_HARDENER_CONF)])
        .await;
    assert!(
        !owned.is_empty(),
