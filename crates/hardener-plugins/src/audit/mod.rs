...
/// The rules file this plugin wants on disk for `config`, and the categories
/// it left out under a valid exception, in file order.
///
/// One renderer for the apply and the dry-run. The apply used to build the
/// content inline and compare it with the file on disk, while the dry-run
/// estimated from which rule categories the kernel had loaded; a template
/// change that kept every category then previewed as nothing and applied as
/// a rewrite (#180). Both now ask this function.
fn rendered_rules_file(config: &PluginConfig) -> (String, Vec<&'static str>) {
    let mut content = String::new();
    content.push_str("# Audit rules generated by Linux Hardening Tool\n");
    content.push_str("# DO NOT EDIT - Changes will be overwritten\n\n");
    content.push_str(AUDIT_BACKLOG_PRELUDE);
...
/// What the dry-run says about the rules file, worded as the apply's own
/// outcome: the apply compares the whole file and writes on any difference,
/// and fails toward "differs" when the file cannot be read (#180). `None`
/// when the file already matches.
async fn rules_file_estimate(ctx: &Context, desired: &str) -> Option<String> {
    let path = Path::new(AUDIT_RULES_PATH);
    // An unanswerable existence probe is read as "exists", the direction that
    // goes on to the read and reports its reason rather than claiming a create.
    if !ctx.executor().path_exists(path).await.unwrap_or(true) {
        return Some(format!("Create {AUDIT_RULES_PATH}"));
    }
    match ctx.executor().read_file(path).await {
        Ok(current) if current == desired => None,
        Ok(_) => Some(format!("Rewrite {AUDIT_RULES_PATH}")),
        // The file is held at 0640, so an unprivileged dry-run lands here on
        // every host the apply has touched; the message says what the apply
        // does when it cannot compare, and why this preview could not.
        Err(e) => Some(format!(
            "Rewrite {AUDIT_RULES_PATH} (an apply that cannot read the current file rewrites it; \
             current content could not be read: {e})"
        )),
    }
}
...
impl HardeningPlugin for AuditHardeningPlugin {
    ...
    async fn apply(&self, ctx: &mut Context, config: &PluginConfig) -> Result<ApplyResult> {
        ...
        // Build rules file content. Rendered by the same function the
        // dry-run compares with (#180), so the two cannot disagree about
        // what the file should hold.
        let (rules_content, skipped_categories) = rendered_rules_file(config);
        for category in skipped_categories {
            let reason = config
                .has_valid_exception(category)
                .map(|exception| exception.reason.clone())
                .unwrap_or_default();
            info!("Skipping audit category '{category}' (exception: {reason})");
            changes.push(Change {
                change_description: format!(
                    "Audit category {category}: skipped (exception: {reason})"
                ),
    ...
    async fn validate(&self, ctx: &Context, config: &PluginConfig) -> Result<ValidationReport> {
        ...
        match probe_auditd(ctx).await {
            AuditdPresence::Installed => {
                ...
                // The loaded set says which categories are live; the file
                // says whether the apply will write. Both are asked, because
                // #180 was a host with every category loaded and a stale file.
                let (desired, _) = rendered_rules_file(config);
                estimated_changes.extend(rules_file_estimate(ctx, &desired).await);
            }
